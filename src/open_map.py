...
try:
	...
	html.write('<script>var map = L.map("map").setView(['+ map_center[1] + ',' + map_center[0] + '], 0);\n')
	html.write('L.tileLayer("' + path + '{z}/{x}/{y}.png", {maxZoom: ' + str(max_zoom) + '}).addTo(map);\n')
	...
	html.write('map.setZoom(' + str(max_zoom) + ');\n')
